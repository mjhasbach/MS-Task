...
function pidOf(procName, callback) {
    if (isString(procName)) {
        ...
        var error = 'The first pidOf() argument provided is not a string (not a valid process name).';
...
function nameOf(procNumber, callback) {
    if(isNumber(procNumber)) {
        ...
        var error = 'The first nameOf() argument provided is non-numeric (not a valid PID).';
        callback(error, null);
    }
}
...
function procStat(proc, callback) {
    var arg = isNumber(proc) ? 'PID' : 'IMAGENAME';
    var cmd = ' /fi \"' + arg + ' eq ' + proc + '\" /fo CSV';
    var data = new ProcObject;

    fileExists(taskListPath, function(exists) {
        if(exists) {
            taskList(cmd, function(err, stdout) {
...
function kill(proc, callback) {
    var arg = isNumber(proc) ? '/PID ' : '/IM ',
        cmd = 'taskkill /F ' + arg + proc,
        err = null;
